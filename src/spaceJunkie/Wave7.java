...
public class Wave7 extends Wave {
	...
	void draw(Graphics g) {
		for (int i =0; i<this.numEnemies;i++){                                  
			...
			this.drawEnemy(g,new Color(0,150,150),(int)this.enemies[i].getPosition().getX(),(int)this.enemies[i].getPosition().getY(),this.enemySize);                       
	...
	void init() {
		...
		this.enemySize = 14;
		this.enemyShotSpeed = 2;
		this.enemyShotProb = .3f;
		for (int i =0; i<(int)(this.numEnemies/2.0);i++){
			this.enemies[i].isDestroyed = false;
			this.enemies[i].setPosition(10+i*15,40+i*20);
			this.enemies[i].getVelocity().setX(1);
		}
		for (int i =(int)(this.numEnemies/2.0); i<this.numEnemies;i++){
			this.enemies[i].isDestroyed = false;;
			this.enemies[i].setPosition(640+(i-(int)(this.numEnemies/2.0))*-15,40+(i-(int)(this.numEnemies/2.0))*20);
			this.enemies[i].getVelocity().setX(-1);
		}
