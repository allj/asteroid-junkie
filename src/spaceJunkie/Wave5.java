 ...
 * It is a group of enemies that
 * moves back and forth, but slowly 
 * gets closer to the player every time 
 * it touches the right side of the screen. 
 */
...
public class Wave5 extends Wave {
	...
	void draw(Graphics g) {
		for (int i =0; i<this.numEnemies;i++){
			...
			if(i>=22) {
				drawEnemy(g,new Color(0,150,255),(int)this.enemyFormation.getPosition().getX()+i*20-421,(int)this.enemyFormation.getPosition().getY()+60,this.enemySize);
				this.enemies[i].setPosition(this.enemyFormation.getPosition().getX()+i*20-421,this.enemyFormation.getPosition().getY()+60);
			} else if (i>=14) {
				drawEnemy(g,new Color(0,150,255),(int)this.enemyFormation.getPosition().getX()+i*20-280,(int)this.enemyFormation.getPosition().getY()+40,this.enemySize);
				this.enemies[i].setPosition(this.enemyFormation.getPosition().getX()+i*20-280,this.enemyFormation.getPosition().getY()+40);
			} else if (i>=6) {
				drawEnemy(g,new Color(0,150,255),(int)this.enemyFormation.getPosition().getX()+i*20-120,(int)this.enemyFormation.getPosition().getY()+20,this.enemySize);
				this.enemies[i].setPosition(this.enemyFormation.getPosition().getX()+i*20-120,this.enemyFormation.getPosition().getY()+20);
			} else {
				drawEnemy(g,new Color(0,150,255),(int)this.enemyFormation.getPosition().getX()+i*20+20,(int)this.enemyFormation.getPosition().getY(),this.enemySize);
	...
	void move(Physics p) {
		...
		if(this.enemyFormation.getPosition().getX() >= SpaceJunkieGame.SCREEN_WIDTH -150) {
			...
			this.enemyFormation.getPosition().setY(this.enemyFormation.getPosition().getY()+5);
		} 
		...
		if(this.waveTime>8) {
			this.enemySpeed = 4.5;
		} else if (this.waveTime>6) {
			this.enemySpeed = 3.5;
		} else if (this.waveTime>4) {
			this.enemySpeed = 2.5;
